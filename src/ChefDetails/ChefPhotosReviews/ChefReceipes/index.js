...
import GetChefPortfolioPictures from './GetChefPortfolioPictures';
...
export const ChefReceipes = (props) => {
  const classes = useStyles();
  const [imageList, setImageList] = React.useState([]);
  
  useEffect(() => {    
    GetChefPortfolioPictures({"Email": props.chefId}).then(function (response) {
        setImageList(response.imageList);
        console.log('GetChefPortfolioPictures', response);
    })
    .catch(function (error) {
      setImageList([]);
        console.log('GetChefPortfolioPictures error', error);
    }); 
  }, []);
